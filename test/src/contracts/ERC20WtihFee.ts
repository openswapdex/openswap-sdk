...
import Bin from "./ERC20WtihFee.json";
...
export class ERC20WtihFee extends _Contract{
    ...
    parseApprovalEvent(receipt: TransactionReceipt): ERC20WtihFee.ApprovalEvent[]{
        return this.parseEvents(receipt, "Approval").map(e=>this.decodeApprovalEvent(e));
    }
    decodeApprovalEvent(event: Event): ERC20WtihFee.ApprovalEvent{
    ...
    parseAuthEvent(receipt: TransactionReceipt): ERC20WtihFee.AuthEvent[]{
        return this.parseEvents(receipt, "Auth").map(e=>this.decodeAuthEvent(e));
    }
    decodeAuthEvent(event: Event): ERC20WtihFee.AuthEvent{
        let result = event.data;
        return {
    ...
    parseTransferEvent(receipt: TransactionReceipt): ERC20WtihFee.TransferEvent[]{
        return this.parseEvents(receipt, "Transfer").map(e=>this.decodeTransferEvent(e));
    }
    decodeTransferEvent(event: Event): ERC20WtihFee.TransferEvent{
        let result = event.data;
        return {
            from: result.from,
            to: result.to,
