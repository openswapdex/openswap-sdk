...
async function deployRestrictedContracts(wallet, options, weth) {
    ...
    let restrictedLiquidityProvider = new contracts_1.OSWAP_RestrictedLiquidityProvider1(wallet);
...
async function initHybridRouterRegistry(wallet, options) {
    let hybridRouterRegistry = new contracts_1.OSWAP_HybridRouterRegistry(wallet, options.registryAddress);
...
function deploy(wallet, options) {
    ...
    return new Promise(async function (resolve, reject) {
        try {
            ...
            let governance = new contracts_1.OAXDEX_Governance(wallet, coreContractsResult.governance);
