...
contract ERC20WithFee is IERC20 {
    ...
    constructor(string memory _symbol, string memory _name, uint256 _initialSupply, uint256 _cap, uint8 _decimals, address _feeTo, uint256 _feeRate) public {
        require(_feeRate <= 1e18, "Invalid fee");
    ...
    function setFee(address _feeTo, uint256 _feeRate) external auth {
        feeTo = _feeTo;
        feeRate = _feeRate;
    }
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        balanceOf[to] = balanceOf[to].add(newValue);
