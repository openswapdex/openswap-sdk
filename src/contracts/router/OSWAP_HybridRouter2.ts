...
export class OSWAP_HybridRouter2 extends Contract{
    ...
    async getPathIn(params:{pair:string[],tokenIn:string}): Promise<any>{
        let result = await this.methods('getPathIn',params.pair,params.tokenIn);
        return result;
    }
    async getPathOut(params:{pair:string[],tokenOut:string}): Promise<any>{
    ...
    async swapETHForExactTokens(params:{amountOut:number|BigNumber,pair:string[],to:string,deadline:number|BigNumber,data:string}): Promise<{path:any,amounts:BigNumber}>{
        let result = await this.methods('swapETHForExactTokens',Utils.toString(params.amountOut),params.pair,params.to,Utils.toString(params.deadline),params.data);
        return {
            path: result.path,
            amounts: new BigNumber(result.amounts)
        }
    }
    async swapExactETHForTokens(params:{amountOutMin:number|BigNumber,pair:string[],to:string,deadline:number|BigNumber,data:string}): Promise<{path:any,amounts:BigNumber}>{
        let result = await this.methods('swapExactETHForTokens',Utils.toString(params.amountOutMin),params.pair,params.to,Utils.toString(params.deadline),params.data);
        return {
            path: result.path,
            amounts: new BigNumber(result.amounts)
        }
    }
    async swapExactETHForTokensSupportingFeeOnTransferTokens(params:{amountOutMin:number|BigNumber,pair:string[],to:string,deadline:number|BigNumber,data:string}): Promise<TransactionReceipt>{
        let result = await this.methods('swapExactETHForTokensSupportingFeeOnTransferTokens',Utils.toString(params.amountOutMin),params.pair,params.to,Utils.toString(params.deadline),params.data);
        return result;
    }
    async swapExactTokensForETH(params:{amountIn:number|BigNumber,amountOutMin:number|BigNumber,pair:string[],to:string,deadline:number|BigNumber,data:string}): Promise<{path:any,amounts:BigNumber}>{
    ...
    async swapExactTokensForTokens(params:{amountIn:number|BigNumber,amountOutMin:number|BigNumber,pair:string[],tokenIn:string,to:string,deadline:number|BigNumber,data:string}): Promise<{path:any,amounts:BigNumber}>{
    ...
    async swapTokensForExactETH(params:{amountOut:number|BigNumber,amountInMax:number|BigNumber,pair:string[],to:string,deadline:number|BigNumber,data:string}): Promise<{path:any,amounts:BigNumber}>{
    ...
    async swapTokensForExactTokens(params:{amountOut:number|BigNumber,amountInMax:number|BigNumber,pair:string[],tokenOut:string,to:string,deadline:number|BigNumber,data:string}): Promise<{path:any,amounts:BigNumber}>{
