...
class OSWAP_HybridRouter extends eth_wallet_1.Contract {
    ...
    async swapETHForExactTokens(params) {
        let result = await this.methods('swapETHForExactTokens', eth_wallet_1.Utils.toString(params.amountOut), params.path, params.to, eth_wallet_1.Utils.toString(params.deadline), params.pair, eth_wallet_1.Utils.toString(params.fee), params.data);
        return new eth_wallet_1.BigNumber(result);
    }
    async swapExactETHForTokens(params) {
        let result = await this.methods('swapExactETHForTokens', eth_wallet_1.Utils.toString(params.amountOutMin), params.path, params.to, eth_wallet_1.Utils.toString(params.deadline), params.pair, eth_wallet_1.Utils.toString(params.fee), params.data);
        return new eth_wallet_1.BigNumber(result);
    }
    async swapExactETHForTokensSupportingFeeOnTransferTokens(params) {
        let result = await this.methods('swapExactETHForTokensSupportingFeeOnTransferTokens', eth_wallet_1.Utils.toString(params.amountOutMin), params.path, params.to, eth_wallet_1.Utils.toString(params.deadline), params.pair, eth_wallet_1.Utils.toString(params.fee));
        return result;
    }
