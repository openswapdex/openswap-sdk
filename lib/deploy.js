...
function toDeploymentContracts(wallet, result) {
    ...
    if (result.rangeFactory) {
        deployment.rangeFactory = new index_1.OSWAP_RangeFactory(wallet, result.rangeFactory);
        deployment.rangeLiquidityProvider = new index_1.OSWAP_RangeLiquidityProvider(wallet, result.rangeLiquidityProvider);
        deployment.executor3 = new index_1.OSWAP_VotingExecutor3(wallet, result.votingExecutor3);
    }
    if (result.restrictedFactory) {
        deployment.configStore = new index_1.OSWAP_ConfigStore(wallet, result.configStore);
        deployment.restrictedFactory = new index_1.OSWAP_RestrictedFactory(wallet, result.restrictedFactory);
        if (result.restrictedLiquidityProvider1) {
            deployment.restrictedLiquidityProvider1 = new index_1.OSWAP_RestrictedLiquidityProvider1(wallet, result.restrictedLiquidityProvider1);
        }
        else if (result.restrictedLiquidityProvider3) {
            deployment.restrictedLiquidityProvider3 = new index_1.OSWAP_RestrictedLiquidityProvider3(wallet, result.restrictedLiquidityProvider3);
        }
        else if (result.restrictedLiquidityProvider4) {
            deployment.restrictedLiquidityProvider4 = new index_1.OSWAP_RestrictedLiquidityProvider4(wallet, result.restrictedLiquidityProvider4);
        }
        deployment.executor4 = new index_1.OSWAP_VotingExecutor4(wallet, result.votingExecutor4);
    }
    return deployment;
}
...
async function deployRestrictedContracts(wallet, options, weth) {
    ...
    let restrictedPairCreator = options.type == 1 ? new index_1.OSWAP_RestrictedPairCreator1(wallet) :
        options.type == 3 ? new index_1.OSWAP_RestrictedPairCreator3(wallet) :
            options.type == 4 ? new index_1.OSWAP_RestrictedPairCreator4(wallet) : null;
    if (!restrictedPairCreator)
        throw new Error("invalid restricted type");
    ...
    let restrictedLiquidityProvider = options.type == 1 ? new index_1.OSWAP_RestrictedLiquidityProvider1(wallet) :
        options.type == 3 ? new index_1.OSWAP_RestrictedLiquidityProvider3(wallet) :
            options.type == 4 ? new index_1.OSWAP_RestrictedLiquidityProvider4(wallet) : null;
