...
export class OSWAP_RestrictedLiquidityProvider extends Contract{
    ...
    async addLiquidityAndTrader(params:{param:number|BigNumber,trader:string[],allocation:number[]|BigNumber[]}): Promise<{pair:string,offerIndex:BigNumber}>{
    ...
    async addLiquidityETH(params:{tokenA:string,addingTokenA:boolean,pairIndex:number|BigNumber,offerIndex:number|BigNumber,amountAIn:number|BigNumber,allowAll:boolean,restrictedPrice:number|BigNumber,startDate:number|BigNumber,expire:number|BigNumber,deadline:number|BigNumber}): Promise<{pair:string,_offerIndex:BigNumber}>{
    ...
    async addLiquidityETHAndTrader(params:{param:number|BigNumber,trader:string[],allocation:number[]|BigNumber[]}): Promise<{pair:string,offerIndex:BigNumber}>{
        let result = await this.methods('addLiquidityETHAndTrader',Utils.toString(params.param),params.trader,Utils.toString(params.allocation));
        return {
            pair: result.pair,
            offerIndex: new BigNumber(result.offerIndex)
        }
    ...
    async removeAllLiquidity(params:{tokenA:string,tokenB:string,to:string,pairIndex:number|BigNumber,deadline:number|BigNumber}): Promise<{amountA:BigNumber,amountB:BigNumber}>{
    ...
    async removeAllLiquidityETH(params:{tokenA:string,to:string,pairIndex:number|BigNumber,deadline:number|BigNumber}): Promise<{amountToken:BigNumber,amountETH:BigNumber}>{
